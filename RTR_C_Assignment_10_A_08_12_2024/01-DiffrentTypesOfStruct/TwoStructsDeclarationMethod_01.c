...
int main(void) {
    ...
    if (point.x == 0 && point.y == 0) {
        ...
        // At least one of the two values (either 'X' or 'Y' or BOTH) is a nonzero value...
        if (point.x == 0) { // If 'X' IS ZERO... OBVIOUSLY 'Y' IS THE NON-ZERO VALUE
            ...
            if (point.x > 0 && point.y > 0) {
                ...
                point_properties.quadrant = 2; // 'X' IS -ve AND 'Y' IS +ve
            } else if (point.x < 0 && point.y < 0) {
                point_properties.quadrant = 3; // 'X' IS -ve AND 'Y' IS -ve
            } else {
                point_properties.quadrant = 4; // 'X' IS +ve AND 'Y' IS -ve
            }
